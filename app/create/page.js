...
import { getUser, getTrips, createPost } from "@/lib/db";
...
export default function CreatePage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (files.length) {
        const urls = await Promise.all(files.map((f) => uploadMedia(f, docRef.id)));
        // update with media urls
        const { updateDoc, doc } = await import("firebase/firestore");
        const { db } = await import("@/lib/firebase");
        await updateDoc(doc(db, "posts", docRef.id), { mediaUrls: urls });
      }
      setDone(true);
      ...
      <form onSubmit={handleSubmit} className="px-4 pt-4 flex flex-col gap-4">
        ...
        <div>
          <p className="text-[10px] font-black tracking-widest uppercase mb-2"
             style={{ color: "var(--color-navy)" }}>Поездка</p>
          <div className="flex flex-col gap-2">
            {trips.length === 0 && (
              <p className="text-[12px]" style={{ color: "var(--color-sub)" }}>
                Поездки добавит учитель в консоли Firebase
              </p>
            )}
            {trips.map((t) => (
              <button
        ...
        <div>
          <p className="text-[10px] font-black tracking-widest uppercase mb-2"
             style={{ color: "var(--color-navy)" }}>Впечатления</p>
          <textarea
            value={text}
        ...
        <div>
          <p className="text-[10px] font-black tracking-widest uppercase mb-2"
             style={{ color: "var(--color-navy)" }}>Настроение</p>
        ...
        <div>
          <p className="text-[10px] font-black tracking-widest uppercase mb-2"
             style={{ color: "var(--color-navy)" }}>Фото и видео</p>
          <button
            type="button"
            ...
            style={{
              ...
              background: "rgba(237,118,21,0.04)",
            }}
          >
          ...
          {previews.length > 0 && (
            <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
              {previews.map((p, i) => (
                <div key={i} className="w-16 h-16 rounded-[10px] flex-shrink-0 overflow-hidden"
        ...
        <button
          type="submit"
          disabled={loading || !tripId || !text.trim()}
        ...
        <div className="rounded-[12px] p-3 flex items-center gap-2.5 mb-4"
             style={{ background: "var(--color-mint)" }}>
